...
export const auth = betterAuth({
  ...
  emailAndPassword: {
    ...
    sendResetPassword: async ({ user, token }, request) => {
      // Fixed hardcoded url
      const baseUrl = process.env.APP_URL || "https://shei-shad-client.vercel.app";
      const secureResetUrl = `${baseUrl}/reset-password?token=${token}&email=${encodeURIComponent(user.email)}`;
  ...
  emailVerification: {
    ...
    sendVerificationEmail: async ({ user, url, token }, request) => {
      try {
        const baseUrl = process.env.APP_URL || "https://shei-shad-client.vercel.app";
        const verifyEmailUrl = `${baseUrl}/verify-email?token=${token}&callbackUrl=/`;
        ...
        await transporter.sendMail({
          ...
          <p>Thanks for creating an account with <strong>Prisma Blog</strong>. Please confirm your email address by clicking the button below:</p>
          <div style="text-align:center;margin:32px 0;">
            <a href="${verifyEmailUrl}" style="background:#2563eb;color:#ffffff;padding:12px 24px;text-decoration:none;border-radius:6px;font-weight:600;display:inline-block;">Verify Email</a>
          </div>
          <p>If the button doesn’t work, copy and paste this link into your browser:</p>
          <p style="word-break:break-all;"><a href="${verifyEmailUrl}">${verifyEmailUrl}</a></p>
          <hr style="margin:24px 0;" />
          <p style="font-size:12px;color:#6b7280;">If you didn’t create this account, you can safely ignore this email.</p>
          <p style="font-size:12px;color:#6b7280;">© ${new Date().getFullYear()} Prisma Blog</p>
        </div>`
        });
      } catch (error: any) {
  ...
  user: {
    additionalFields: {
      ...
      status: { type: "string", defaultValue: "ACTIVATE", required: false },
      deliveryAddress: { type: "string", defaultValue: "", required: false }
    }
  },
