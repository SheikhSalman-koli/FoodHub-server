...
export const getProviderStats = async (email) => {
    // console.log(email);
    try {
    ...
    `;
      ...
      ORDER BY month ASC;
    `;
        const formatChartData = (data, keyName, type) => data.map((item) => ({
...
export const getAdminDashboardStats = async () => {
    const [allOrders, totalCustomersCount, totalProvidersCount, recentOrders, topMealGroups,] = await Promise.all([
    ...
    allOrders.forEach((order) => {
        ...
        if (!weeklyMap[weekKey])
            weeklyMap[weekKey] = { revenue: 0, orders: 0 };
        weeklyMap[weekKey].orders += 1;
        if (order.status === "DELIVERED")
            weeklyMap[weekKey].revenue += amount;
